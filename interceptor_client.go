...
func fileClientWithLineNum() string {
	...
	for i := 2; i < 15; i++ {
		...
		if (!strings.HasSuffix(file, "ego-component/ekafka/interceptor_client.go") && !strings.Contains(file, "/ego-component/ekafka/producer.go")) || strings.HasSuffix(file, "_test.go") {
